package com.cxone.restclient.httpClient.utils;
...
public abstract class HttpClientHelper {
    ...
    public static <T> T convertToObject(HttpResponse response, Class<T> responseType, boolean isCollection) throws IOException, CxOneClientException {
    ...
    private static <T> T convertToStrObject(HttpResponse response, Class<T> valueType) throws CxClientException {
        ...
        try {
            ...
            throw new CxOneClientException("Failed to parse json response: " + e.getMessage());
        }
    }

    public static String convertToJson(Object o) throws CxOneClientException {
        ObjectMapper mapper = getObjectMapper();
        try {
            return mapper.writeValueAsString(o);
        } catch (Exception e) {
            throw new CxOneClientException("Failed convert object to json: " + e.getMessage());
        }
    }

    public static StringEntity convertToStringEntity(Object o) throws CxOneClientException, UnsupportedEncodingException {
        return new StringEntity(convertToJson(o));
    }

    private static <T> T convertToCollectionObject(HttpResponse response, JavaType javaType) throws CxOneClientException {
        ObjectMapper mapper = getObjectMapper();
        try {
            String json = IOUtils.toString(response.getEntity().getContent(), Charset.defaultCharset());
            return mapper.readValue(json, javaType);
        } catch (IOException e) {
            throw new CxOneClientException("Failed to parse json response: " + e.getMessage(), e);
        }
    }
    ...
    public static void validateResponse(HttpResponse response, int expectedStatus, String message) throws CxClientException {
        ...
        if (actualStatusCode != expectedStatus) {
            ...
            throw new CxOneHTTPClientException(actualStatusCode, exceptionMessage, responseBody);
        }
    }
